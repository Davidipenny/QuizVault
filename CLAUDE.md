...
**Collection dual-format storage**: Collections support two storage formats for backward compatibility:
- **Old format** (via `save_bank_collection`): direct list of question dicts `[{id, type, question, ...}, ...]`
- **Bank mode format** (via `create_bank_collection` + `add_to_bank_collection`): dict with metadata `{"created": "...", "questions": [{"id": 1, "type": "single", "added_at": "..."}]}`
The helper `_get_collection_questions()` in `collection.py` normalizes both formats. When modifying collection code, handle both shapes.
...
**Wrong-question deduplication**: Uses `(id, type)` composite keys. `save_wrong_question()` checks for existing entries by this key and increments `wrong_count` instead of duplicating. `remove_bank_wrong_question()` removes from ALL wrong books simultaneously when a question is answered correctly.
